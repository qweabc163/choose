...
"""
A股趋势策略增强版：
指定日期股票分类 + 未来走势回测 + 动态卖出验证

增强内容：
1. 增加 MA5 / MA10；
2. MA20 / MA60 / MA120 判断中期趋势；
3. MA5 / MA10 判断短线强度、短线走弱、回踩修复；
4. 增加大盘环境过滤；
5. 增加可选行业强度过滤；
6. 增加 ATR20；
7. 动态卖出支持 ATR 止损；
8. 动态卖出支持跌破 MA10 且短线走弱；
9. 增加手续费、印花税、滑点；
10. 增加简单涨停买不进、跌停卖不出处理；
11. 默认使用本地 data/*.csv，不联网；
12. 输出 Excel 和 CSV。

使用示例：

仅分类：
python pool04_enhanced.py --date 2026-04-30 --classify-only

回测：
python pool04_enhanced.py --date 2026-04-30 --days 30

非交易日自动前移：
python pool04_enhanced.py --date 2026-05-01 --use-prev-trading-day --classify-only
"""
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 回踩修，使用最近5日低点接近 MA10 或 MA20，且收盘重新站上 MA10
...
def judge_weakness_for_exit(
    ...
    ma5_slope_3 = row.get("ma5_slope_3", np.nan)
    ma10_slope_5 = row.get("ma10_slope_5", np.nan)
    ma20_slope_5 = row.get("ma20_slope_5", np.nan)
    ret10 = row.get("ret10", np.nan)
    atr20 = row.get("atr20", np.nan)
    amount = row.get("amount", np.nan)
    amount_ma20 = row.get("amount_ma20", np.nan)
    heavy_bearish = bool(row.get("heavy_bearish_candle", False))

    if pd.isna(close) or close <= 0 or pd.isna(raw_buy_price) or raw_buy_price <= 0:
        return None

    current_ret = calc_net_return(close, raw_buy_price)

    # 固定硬止损
    if not pd.isna(current_ret) and current_ret <= stop_loss_pct:
        return f"硬止损{stop_loss_pct:.0%}"

    # ATR止损
    if ATR_STOP_ENABLED and not pd.isna(atr20) and atr20 > 0:
        atr_stop_price = raw_buy_price - ATR_STOP_MULT * atr20
        if close <= atr_stop_price:
            return f"ATR止损{ATR_STOP_MULT:.1f}倍"

    # 跌破 MA60，趋势破坏，不受最少持有天数限制
    ...
    if not pd.isna(peak_close) and peak_close > 0:
        peak_ret = calc_net_return(peak_close, raw_buy_price)
        drawdown_from_peak = close / peak_close - 1

        if not pd.isna(peak_ret):
            if peak_ret >= trailing_start_pct and drawdown_from_peak <= -trailing_drawdown_pct:
                return f"移动止盈回撤{trailing_drawdown_pct:.0%}"

        # ATR移动止盈
        if ATR_STOP_ENABLED and not pd.isna(atr20) and atr20 > 0:
            if not pd.isna(peak_ret) and peak_ret >= trailing_start_pct:
                if close <= peak_close - ATR_TRAILING_MULT * atr20:
                    return f"ATR移动止盈{ATR_TRAILING_MULT:.1f}倍"
    ...
    if not pd.isna(amount) and not pd.isna(amount_ma20) and amount_ma20 > 0:
        volume_confirm = amount >= amount_ma20 * 1.1

    if (
        not pd.isna(ma5)
        and not pd.isna(ma10)
        and not pd.isna(ma5_slope_3)
        and close < ma10
        and ma5 < ma10
        and ma5_slope_3 <= 0
        and volume_confirm
...
def calc_forward_trade_with_dynamic_exit(
    ...
    result = {
        ...
        "dynamic_raw_buy_price": raw_buy_price,
    ...
    for i, d in enumerate(forward_dates, start=1):
        ...
        # pending_exit 表示上一日触发，今天开盘卖
        if (not exited) and pending_exit:
            # 如果今天开盘跌停，模拟卖不出去，继续等待
            if is_open_limit_down(row):
                strategy_ret = calc_net_return(close_price, raw_buy_price)
                result[f"strategy_D{i}"] = strategy_ret
                strategy_returns.append(strategy_ret)
                continue
            ...
            if not pd.isna(open_price) and open_price > 0:
                sell_date = d
                raw_sell_price = open_price
                sell_price = net_sell_price(raw_sell_price)
                sell_day = i
    ...
    if not exited:
        if not pd.isna(last_valid_close) and last_valid_close > 0:
            ...
            raw_sell_price = last_valid_close
            sell_price = net_sell_price(raw_sell_price)
    ...
    result["dynamic_raw_sell_price"] = raw_sell_price
...
def style_worksheet(
    ...
    if money_keywords is None:
        money_keywords = ["成交额", "买入价", "卖出价", "收盘", "MA", "ATR"]
...
def save_classification_excel(
    ...
    dashboard_rows.append(["ATR止损", "开启" if ATR_STOP_ENABLED else "关闭", f"{ATR_STOP_MULT}倍ATR"])
...
def build_return_matrix_df(detail_df: pd.DataFrame, forward_days: int) -> pd.DataFrame:
    ...
    cols = ["code", "name", "industry", "status", "score", "buy_date", "buy_price"]
...
def build_excess_matrix_df(excess_df: pd.DataFrame, forward_days: int) -> pd.DataFrame:
    ...
    cols = ["code", "name", "industry", "status", "score", "buy_date", "avg_excess", "outperform_days", "valid_days"]
...
def make_dynamic_exit_readable(dynamic_exit_df: pd.DataFrame, forward_days: int) -> pd.DataFrame:
    ...
    keep_cols = [
        "code", "name", "industry", "status", "score", "signal_date",
        "dynamic_buy_date", "dynamic_raw_buy_price", "dynamic_buy_price",
        "dynamic_sell_date", "dynamic_raw_sell_price", "dynamic_sell_price",
        ...
        "amount_ratio_5_20", "max_dd20", "atr20_pct", "dist_to_60d_high",
    ]
    ...
    rename_map = {
        ...
        "dynamic_raw_buy_price": "原始买入价",
        "dynamic_buy_price": "买入价含成本",
        "dynamic_sell_date": "卖出日",
        "dynamic_raw_sell_price": "原始卖出价",
        "dynamic_sell_price": "卖出价扣成本",
...
def save_friendly_excel(
    ...
    dashboard_rows.append(["大盘环境过滤", "开启" if MARKET_FILTER_ENABLED else "关闭", "沪深300趋势过滤"])
    dashboard_rows.append(["行业强度过滤", "开启" if INDUSTRY_FILTER_ENABLED else "关闭", "可选行业强度"])
    dashboard_rows.append(["涨跌停处理", "开启" if LIMIT_CHECK_ENABLED else "关闭", "涨停买不进，跌停卖不出"])
    dashboard_rows.append(["买入佣金", BUY_COMMISSION_RATE, "交易成本"])
    dashboard_rows.append(["卖出佣金", SELL_COMMISSION_RATE, "交易成本"])
    dashboard_rows.append(["印花税", STAMP_TAX_RATE, "仅卖出"])
    dashboard_rows.append(["滑点", SLIPPAGE_RATE, "买入加，卖出减"])
    dashboard_rows.append(["ATR止损", "开启" if ATR_STOP_ENABLED else "关闭", f"{ATR_STOP_MULT}倍ATR20"])
...
def analyze_pool_classification(
    ...
    selected_df = pool_df[pool_df["status"].isin(["强趋势池", "趋势观察池", "回踩修复观察"])].copy()
    ...
    if not selected_df.empty:
        preview_cols = [
            ...
            "short_term_strong", "pullback_rebound", "dist_to_60d_high",
        ]
...
def build_dynamic_exit_detail(
    ...
    for _, stock in selected_df.iterrows():
        ...
        row = {
            ...
            "atr20_pct": stock.get("atr20_pct", np.nan),
...
def analyze_pool_forward_returns(
    ...
    preview_cols = [
        "code", "name", "industry", "status", "score", "ret20", "ret60",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="增强版：指定日期股票分类 / 股票池未来走势分析"
    )
...
def main():
    ...
    global ATR_STOP_ENABLED
    global ATR_STOP_MULT
    global ATR_TRAILING_MULT
    ...
    ATR_STOP_ENABLED = not args.no_atr_stop
    ...
    print(f"ATR止损: {'开启' if ATR_STOP_ENABLED else '关闭'}")
    print(f"ATR止损倍数: {ATR_STOP_MULT}")
    print(f"ATR移动止盈倍数: {ATR_TRAILING_MULT}")
